...
`,
  ...
  function createChannel(streamIn) {
    ...
    let transform2 = ({ callName, refs, input, options, isTTY, fs, callback }) => {
      ...
      if (
        (typeof input === "string" || input instanceof Uint8Array) && input.length > 1024 * 1024
      ) {
        let next = start;
  ...
  var handlePlugins = (
    ...
    __async(null, null, function*() {
      ...
      requestCallbacks["on-start"] = (id, request) =>
        __async(null, null, function*() {
          details.clear();
          let response = { errors: [], warnings: [] };
          yield Promise.all(onStartCallbacks.map(_0 =>
            __async(null, [_0], function*({ name, callback, note }) {
              try {
      ...
      requestCallbacks["on-resolve"] = (id, request) =>
        __async(null, null, function*() {
      ...
      requestCallbacks["on-load"] = (id, request) =>
        __async(null, null, function*() {
          let response = {}, name = "", callback, note;
          for (let id2 of request.ids) {
            try {
...
${file}:${line}:${column}: ERROR: ${pluginText}${e.text}`;
  ...
  function sanitizeLocation(location2, where, terminalWidth) {
    ...
    if (lineText) {
      const relevantASCII = lineText.slice(
        0,
        (column && column > 0 ? column : 0) + (length && length > 0 ? length : 0)
  ...
  var startRunningService = (wasmURL, wasmModule, useWorker) =>
    __async(null, null, function*() {
      let worker;
      let rejectAllWith;
