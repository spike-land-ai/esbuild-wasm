...
function flagsForBuildOptions(
  ...
  let write = (_a2 = getFlag(options, keys, "write", mustBeBoolean)) != null
    ? _a2
    : writeDefault;
  ...
  let handleIncomingPacket = bytes => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.27.3") {
        throw new Error(
          `Cannot start service: Host version "${"0.27.3"}" does not match binary version ${
            quote(binaryVersion)
          }`,
        );
      }
          ...
          let next = () => {
            if (--outstanding === 0) {
              ...
              if ("legalComments" in response) {
                result.legalComments = response == null
                  ? void 0
                  : response.legalComments;
              }
              if (response.mangleCache) {
                result.mangleCache = response == null
                  ? void 0
                  : response.mangleCache;
              }
...
var isInternalWorkerThread =
  ((_a = worker_threads == null ? void 0 : worker_threads.workerData) == null
    ? void 0
    : _a.esbuildVersion) === "0.27.3";
var esbuildCommandAndArgs = () => {
  // 1. Honor ESBUILD_BINARY_PATH env var (was dead code due to `|| true`)
  if (ESBUILD_BINARY_PATH) {
    return ["node", [ESBUILD_BINARY_PATH]];
  }
  // 2. Try __dirname-based resolution (works when not virtualized)
  var binPath = path2.join(__dirname, "..", "bin", "esbuild");
  try {
  ...
  try {
    var resolved = require.resolve("@spike-land-ai/esbuild-wasm/package.json");
    binPath = path2.join(path2.dirname(resolved), "bin", "esbuild");
    if (fs2.existsSync(binPath)) {
      return ["node", [binPath]];
    }
  } catch {}
  ...
  writeFile(contents, callback) {
    try {
      ...
      fs2.writeFile(
        tempFile,
        contents,
        err => err !== null ? callback(null) : callback(tempFile),
      );
    } catch {
      callback(null);
    }
...
var version = "0.27.3"; // version sourced from package.json build step
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(
    command,
    args.concat(`--service=${"0.27.3"}`, "--ping"),
    {
      windowsHide: true,
...
var startWorkerThreadService = worker_threads2 => {
  let { port1: mainPort, port2: workerPort } = new worker_threads2
    .MessageChannel();
...
error: ${text}`);
  ...
  let runCallSync = (command, args) => {
    ...
    let { message: { id: id2, resolve, reject, properties } } = worker_threads2
